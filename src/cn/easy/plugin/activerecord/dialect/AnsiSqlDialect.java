...
package cn.easy.plugin.activerecord.dialect;
...
import cn.easy.plugin.activerecord.ActiveRecordException;
import cn.easy.plugin.activerecord.CPI;
import cn.easy.plugin.activerecord.DbUtil;
import cn.easy.plugin.activerecord.Model;
import cn.easy.plugin.activerecord.ModelBuilder;
import cn.easy.plugin.activerecord.Page;
import cn.easy.plugin.activerecord.Record;
import cn.easy.plugin.activerecord.Table;
...
public class AnsiSqlDialect extends Dialect {
	...
	/**
	 * SELECT * FROM subject t1 WHERE (SELECT count(*) FROM subject t2 WHERE
	 * t2.id < t1.id AND t2.key = '123') > = 10 AND (SELECT count(*) FROM
	 * subject t2 WHERE t2.id < t1.id AND t2.key = '123') < 20 AND t1.key =
	 * '123'
	 */
	...
	public Page<Record> takeOverDbPaginate(Connection conn, int pageNumber,
			int pageSize, String select, String sqlExceptSelect,
			Object... paras) throws SQLException {
		long totalRow = 0;
		int totalPage = 0;
		List result = CPI.query(
				conn,
				"select count(*) "
		...
		if (pst != null)
			pst.close();
		return new Page<Record>(list, pageNumber, pageSize, totalPage,
				(int) totalRow);
	}

	private List<Record> buildRecord(ResultSet rs, int pageSize)
			throws SQLException {
	...
	private void buildLabelNamesAndTypes(ResultSetMetaData rsmd,
	...
	public Page<? extends Model> takeOverModelPaginate(Connection conn,
			Class<? extends Model> modelClass, int pageNumber, int pageSize,
		...
		if (size == 1)
			...
			return new Page(new ArrayList(0), pageNumber, pageSize, 0, 0); // totalRow
																			// =
																			// 0;
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public final <T> List<T> buildModel(ResultSet rs,
			Class<? extends Model> modelClass, int pageSize)
